...
**Taglines in use:** "split your session at the silences. keep the good takes." (hero, and the OG image verbatim) · "take triage for voice actors" (descriptor) · "corta tu sesión en los silencios. quédate con las tomas buenas." (Spanish hero).
...
Detection: silence threshold, minimum silence, margin, fade at cuts, re-detect. Listening: monitor gain, slow-motion speed, slow motion on `↑`, autoplay, line mode wraps around (on). Transcription: language (Spanish, English, auto), model (small, base), empty takes go to junk, re-cut takes with several reads. Editing: frame step, arrows move fast, split stays on the first half, context around a clip. Lanes and look: lane names, theme (auto, dark, light), interface language (auto, English, Spanish). Export: order (by line / by time), silent gaps between takes (on), gap after every take (0.75 s), gap between lines (1.5 s), file name prefix.
